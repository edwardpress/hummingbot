...
class EunionAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_last_traded_prices(cls, trading_pairs: List[str]) -> Dict[str, float]:
        ...
        async with aiohttp.ClientSession() as client:
            resp = await client.get(EUNION_TICKER_URL)
    ...
    async def get_snapshot(client: aiohttp.ClientSession, trading_pair: str) -> Dict[str, Any]:
        ...
        async with client.get(EUNION_DEPTH_URL, params=params) as response:
            response: aiohttp.ClientResponse = response
            if response.status != 200:
                raise IOError(f"Error fetching Eunion market snapshot for {trading_pair}. "
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        async with aiohttp.ClientSession() as client:
            ...
            snapshot_msg: OrderBookMessage = EunionOrderBook.snapshot_message_from_exchange(
                snapshot,
    ...
    async def listen_for_trades(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        while True:
            try:
                ...
                async with websockets.connect(EUNION_WS_URI) as ws:
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        #  compresses their ws data
                        encoded_msg: bytes = gzip.decompress(raw_msg)
                        #  data value for id is a large int too big for ujson to parse
                        ...
                        if "ping" in msg:
                            ...
                            for data in msg["tick"]["data"]:
                                trade_message: OrderBookMessage = EunionOrderBook.trade_message_from_exchange(
                                    data, metadata={"trading_pair": trading_pair}
                                )
                                output.put_nowait(trade_message)
                        else:
                            self.logger().debug(f"Unrecognized message received from Eunion websocket: {msg}")
            except asyncio.CancelledError:
                raise
            except Exception:
                self.logger().error("Unexpected error with Eunion WebSocket connection. Retrying after 30 seconds...",
                                    exc_info=True)
                ...
                async with websockets.connect(EUNION_WS_URI) as ws:
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        ...
                        if "ping" in msg:
                            ...
                            order_book_message: OrderBookMessage = EunionOrderBook.diff_message_from_exchange(msg)
            ...
            try:
                ...
                async with aiohttp.ClientSession() as client:
                    for trading_pair in trading_pairs:
                        try:
                            snapshot: Dict[str, Any] = await self.get_snapshot(client, trading_pair)
                            snapshot_message: OrderBookMessage = EunionOrderBook.snapshot_message_from_exchange(
                                snapshot,
                                metadata={"trading_pair": trading_pair}
                            )
